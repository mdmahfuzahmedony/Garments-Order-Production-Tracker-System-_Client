...
import { FaUser, FaEnvelope, FaIdBadge, FaCalendarAlt, FaMapMarkerAlt, FaShoppingBag, FaPhone } from "react-icons/fa";
...
const MyProfile = () => {
  ...
  const roleBadgeColor = dbUser?.role === 'manager' ? 'badge-secondary' : dbUser?.role === 'admin' ? 'badge-error' : 'badge-warning';
    ...
    <div className="w-full max-w-6xl mx-auto p-4 lg:p-8">
      ...
      <div className="relative bg-gradient-to-r from-blue-600 to-cyan-400 rounded-b-none rounded-t-2xl h-48 mb-20 shadow-lg">
        ...
        <div className="absolute top-4 right-4">
            <div className={`badge ${roleBadgeColor} badge-lg font-bold uppercase shadow-md p-4 text-white tracking-wider`}>
                {dbUser?.role || "Buyer"}
            </div>
        </div>
            ...
            <div className="card-body">
                ...
                <div className="space-y-4 text-sm lg:text-base">
                    <div className="flex justify-between items-center bg-base-100 p-3 rounded-lg">
                        ...
                        <span className="font-mono text-xs md:text-sm">{dbUser?._id}</span>
                    </div>
                    <div className="flex justify-between items-center bg-base-100 p-3 rounded-lg">
                        <span className="font-semibold text-gray-500">Status</span> 
                        <span className="badge badge-success badge-outline font-bold">Active</span>
                    </div>
                    <div className="flex justify-between items-center bg-base-100 p-3 rounded-lg">
                ...
                <div className="space-y-4 text-sm lg:text-base">
                    ...
                    <div className="flex items-center gap-3 bg-base-100 p-3 rounded-lg">
                        ...
                        <div>
                            ...
                            <p className="font-semibold italic text-gray-400">Not set yet</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3 bg-base-100 p-3 rounded-lg">
                        ...
                        <div>
                            ...
                            <p className="font-semibold">{user?.metadata?.lastSignInTime ? new Date(user.metadata.lastSignInTime).toLocaleDateString() : "Today"}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex justify-center">
                {!dbUser.requestedRole ? (
                    <div className="w-full max-w-3xl bg-blue-50 border border-blue-200 rounded-xl p-6 flex flex-col md:flex-row items-center justify-between gap-4 shadow-md">
                        <div>
                            <h4 className="text-lg font-bold text-blue-900">Interested in Selling Products?</h4>
                            <p className="text-sm text-blue-700">Upgrade your account to Manager to access the seller dashboard.</p>
                        </div>
                        <button 
                    ...
                    <div className="w-full max-w-3xl bg-yellow-50 border border-yellow-200 rounded-xl p-6 flex items-center justify-center gap-3 shadow-md">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-yellow-700">Your request to become a Manager is under review.</p>
                        </div>
                    </div>
                )}
            </div>
